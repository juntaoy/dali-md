...
import pyhocon
import lee_md,biaffine_md,bert_md

def get_model(config):
  model_type = config["model_type"]
  if model_type == 'lee':
    print('Using LEE MD')
    model = lee_md.LeeMD(config)
  elif model_type == 'biaffine':
    print('Using BIAFFINE MD')
    model = biaffine_md.BiaffineMD(config)
  elif model_type == 'bert':
    print('Using BERT MD')
    model = bert_md.BertMD(config)
  else:
    print('Unknown model_type: %s! model avaiable: lee,biaffine,bert' % model_type)
...
def bilinear_classifier(x_bnv, y_bnv, keep_prob, add_bias_1=True, add_bias_2=True):
